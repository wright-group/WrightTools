...
# Step 1: get pyro channels
    ...
    # Step 3: find "chi" for 2w1+w3
        ...
        off_pyro1  = np.array([shots[index_dict['pyro1']][i] for i in range(len(shots[index_dict['pyro1']])) if not mask[i]])
        ...
        chi = np.mean(off_signal*off_sig_z/(off_pyro1*off_pyro1*off_pyro3))
        if chi < 0:
            chi = 0
    ...
    # Step 4: Normalize after (not) subtracting the appropreate chi*p1*p1*p3 value
        ...
        norm_signal_clever = np.mean(on_signal*on_sig_z/(on_pyro1*on_pyro2*on_pyro3)-chi*on_pyro1*on_pyro1*on_pyro3)
    ...
    # Raw and (Bulk) Normalized Photon Counting
        ...
        norm_fac = out[out_names.index('pyro1_mean')]*out[out_names.index('pyro2_cmean')]*out[out_names.index('pyro3_mean')]
