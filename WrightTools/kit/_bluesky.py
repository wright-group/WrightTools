...
def match_identifier(dir:pathlib.Path, **bluesky_identifier) -> list[BlueskyFolder]:
    ...
    for info in map(
        ...
        filter(
            lambda item: item.is_dir() and re.fullmatch(__folder_seed__, item.name),
            dir.iterdir()
        )
    ):
        idict = info._asdict()
...
def parse_folder_name(folder:str) -> FolderInfo | None:
    ...
    if ((uid_match := re.fullmatch(r"(?P<uid>\w{8})", folder.split()[-1])) is not None) \
        and ((datetime_match := __datetime_seed__.match(folder)) is not None):
