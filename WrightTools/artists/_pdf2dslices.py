...
class PDF2DSlices:
    ...
    def _fill_plot(self, xaxis, yaxis, zi, ax, cax, title, yticks, vmin=None, vmax=None):
        xi = xaxis[:]
        yi = yaxis[:]
    ...
    def _fill_row(self, data, channel_index, gs, row_index, global_limits):
        ...
        if not self.data_signed:
            kwargs["vmin"] = vmin
        sps0 = self._fill_plot(
            xaxis, yaxis, zi, ax0, cax, title=data.name + " local", yticks=True, **kwargs
        )
        # global
        ax1 = plt.subplot(gs[row_index, 3])
        cax = plt.subplot(gs[row_index, 4])
        zi = data.channels[channel_index][:]
        sps1 = self._fill_plot(
            xaxis,
            yaxis,
            zi,
            ax1,
    ...
    def plot(self, channel=0, output_path=None, w1w2=True, w1_wigner=True, w2_wigner=True):
        """Plot."""
        # get channel index
        if type(channel) in [int, float]:
            channel_index = int(channel)
            ...
            if w1w2:
                ...
                for slice_index in range(len(self.chopped_datas[0])):  # for each chop...
                    ...
                    fig, gs = create_figure(
                        width="double", nrows=len(self.datas), cols=cols, hspace=0.5
                    )
                    for data_index in range(len(self.datas)):
                        data = self.chopped_datas[data_index][slice_index]
                        if self.data_signed:
                            global_limits = [
                                self.datas[data_index].channels[channel_index].min(),
                                self.datas[data_index].channels[channel_index].max(),
                            ]
                        else:
                            global_limits = [
                                self.datas[data_index].channels[channel_index].null(),
                                self.datas[data_index].channels[channel_index].max(),
                            ]
